...
private let logger = Logger(label: "AnyLanguageModel.Models.OpenAILanguageModel")
...
public struct OpenAILanguageModel: LanguageModel {
    ...
    public func respond<Content>(
        ...
        let openAITools = convertTools(from: session)
    ...
    private func respondWithChatCompletions<Content>(
        ...
        logger.info(
            "Starting ChatCompletions request",
            metadata: [
                "model": "\(model)",
        ...
        while true {
            // Build complete transcript for this API call (session transcript + accumulated entries)
            let currentTranscript = Array(session.transcript) + entries

            // Notify session that an API request is starting
            let apiStartTime = Date()
            let eventID = session.notifyAPIRequestStarted(
            ...
            logger.debug(
                "Sending ChatCompletions request",
                metadata: [
                    "url": "\(url)",
                    "messages": "\((messages.last?.content as? String) ?? "N/A")",
                ]
            )
            ...
            if let toolCalls = toolCallMessage.toolCalls, !toolCalls.isEmpty {
                logger.debug("Tool calls detected", metadata: ["count": "\(toolCalls.count)"])

                // Emit API request completed event (partial response with tool calls)
                let partialText = toolCallMessage.content ?? "(tool calls)"
                session.notifyAPIRequestCompleted(
                    eventID: eventID,
                    duration: apiDuration,
                    content: partialText,
                    tokenUsage: resp.usage,
                    metadata: resp.usage.map { ResponseMetadata(tokenUsage: $0) }
                )
            ...
            // Emit API request completed event (final response)
            let metadata = resp.usage.map { ResponseMetadata(tokenUsage: $0) }
            session.notifyAPIRequestCompleted(
                eventID: eventID,
                duration: apiDuration,
                content: text,
            ...
            return try parseResponseContent(
                from: text,
                type: type,
                entries: entries,
                metadata: metadata,
                debugPrefix: "ChatCompletions"
            )
        }
    ...
    private func respondWithResponses<Content>(
        ...
        while true {
            ...
            logger.debug("Sending Responses API request", metadata: ["url": "\(url)"])

            // Emit API request started event
            let eventID = session.notifyAPIRequestStarted(
                transcriptEntries: currentTranscript,
                options: options,
                messages: messages.count
            ...
            if !toolCalls.isEmpty {
                logger.debug("Tool calls detected in Responses API", metadata: ["count": "\(toolCalls.count)"])

                // Emit API request completed event (partial response with tool calls)
                let partialText = resp.outputText ?? extractTextFromOutput(resp.output) ?? "(tool calls)"
                session.notifyAPIRequestCompleted(
                    eventID: eventID,
                    duration: apiDuration,
                    content: partialText,
    ...
    private func convertTools(from session: LanguageModelSession) -> [OpenAITool]? {
    ...
    private func prepareMessages<Content>(
        from session: LanguageModelSession,
        type: Content.Type,
        includeSchemaInPrompt: Bool,
        debugPrefix: String = ""
    ) -> [OpenAIMessage] where Content: Generable {
        var messages = session.transcript.toOpenAIMessages()
        let isStringType = type == String.self

        if !isStringType && includeSchemaInPrompt {
            let schema = type.generationSchema
            let resolvedSchema = schema.withResolvedRoot() ?? schema

            let encoder = JSONEncoder()
            encoder.outputFormatting = [.prettyPrinted, .sortedKeys]
            if let schemaData = try? encoder.encode(resolvedSchema),
                let schemaJSON = String(data: schemaData, encoding: .utf8)
            {
                ...
                messages.insert(
                    OpenAIMessage(
                        role: .system,
                        content: .text("You must respond with valid JSON matching this exact schema:\n\(schemaJSON)")
                    ),
                    at: 0
                )
            }
    ...
    private func parseResponseContent<Content>(
        from text: String,
        type: Content.Type,
            ...
            if !debugPrefix.isEmpty {
                logger.debug(
                    ...
                    metadata: [
                        "context": "\(debugPrefix)",
                        "type": "\(type)",
                        "textPreview": "\(text.prefix(200))",
                    ]
                )
            }
            // Parse JSON for custom types
            do {
                let generatedContent = try GeneratedContent(json: text)
                let content = try type.init(generatedContent)
                if !debugPrefix.isEmpty {
                    logger.info("Successfully parsed custom type", metadata: ["context": "\(debugPrefix)"])
                }
                return LanguageModelSession.Response(
                    content: content,
                    rawContent: generatedContent,
                ...
                if !debugPrefix.isEmpty {
                    logger.debug(
                        "Initial JSON parsing failed, attempting partial decoding",
                        metadata: [
                            "context": "\(debugPrefix)",
                ...
                // Attempt partial JSON decoding
                let decoder = PartialJSONDecoder()
                if let partialContent = try? decoder.decode(GeneratedContent.self, from: text).value,
                    let content = try? type.init(partialContent)
                {
                    if !debugPrefix.isEmpty {
                        logger.debug("Partial JSON decoding succeeded", metadata: ["context": "\(debugPrefix)"])
                    }
                    return LanguageModelSession.Response(
                        content: content,
                ...
                if !debugPrefix.isEmpty {
                    logger.warning(
                        "Both JSON parsing and partial decoding failed",
                        metadata: [
                            "context": "\(debugPrefix)",
                            "error": "\(String(describing: error))",
                        ]
    ...
    private func yieldStreamSnapshot<Content>(
        _ text: String,
        isStringType: Bool,
        ...
        if isStringType {
            ...
            let content: Content.PartiallyGenerated = (text as! Content).asPartiallyGenerated()
            continuation.yield(.init(content: content, rawContent: raw))
        } else {
            let decoder = PartialJSONDecoder()
            if let partialContent = try? decoder.decode(GeneratedContent.self, from: text).value {
                if let partial: Content.PartiallyGenerated = try? Content.PartiallyGenerated.init(partialContent) {
                    logger.debug("Streaming partial JSON snapshot", metadata: ["charCount": "\(text.count)"])
                    continuation.yield(.init(content: partial, rawContent: partialContent))
                } else {
                    logger.debug("Failed to create PartiallyGenerated from content")
                }
            } else {
                // Silently skip if partial JSON is not yet valid
            }
        }
    }
    ...
    public func streamResponse<Content>(
        ...
        let messages = prepareMessages(
            ...
            type: type,
            includeSchemaInPrompt: includeSchemaInPrompt,
            debugPrefix: "Streaming"
        )
        let isStringType = type == String.self

        switch apiVariant {
        case .responses:
            logger.debug("Starting Responses API streaming")
            let params = Responses.createRequestBody(
                model: model,
                messages: messages,
                tools: openAITools,
            ...
            let url = baseURL.appendingPathComponent("responses")
            let capturedIsStringType = isStringType
                    ...
                    do {
                        ...
                        for try await event in events {
                            ...
                            case .outputTextDelta(let delta):
                                ...
                                yieldStreamSnapshot(
                                    accumulatedText,
                                    isStringType: capturedIsStringType,
        ...
        case .chatCompletions:
            logger.debug("Starting ChatCompletions streaming")
            let params = ChatCompletions.createRequestBody(
                model: model,
                messages: messages,
                tools: openAITools,
...
private func convertSegmentsToOpenAIBlocks(_ segments: [Transcript.Segment]) -> [Block] {
    ...
    for segment in segments {
        ...
        case .toolCalls:
            // Tool calls are handled separately as transcript entries
            break
        }
    }
    return blocks
...
private func resolveToolCalls(
    ...
    logger.debug("Resolving tool calls", metadata: ["count": "\(toolCalls.count)"])
    ...
    for (index, call) in toolCalls.enumerated() {
        ...
        guard let tool = toolsByName[function.name] else {
            logger.warning("Tool not found", metadata: ["toolName": "\(function.name)"])
        ...
        do {
            let segments = try await session.executeToolWithEvents(tool, arguments: args, callIndex: index)
            let output = Transcript.ToolOutput(
                id: callID,
                toolName: tool.name,
                segments: segments
            ...
            // Log the content returned by the tool
            let segmentDescriptions = segments.map { segment -> String in
                switch segment {
                case .text(let text):
                    return "text(\(text.content.prefix(100)))"
                case .structure(let structured):
                    return "structure(\(structured.content.jsonString.prefix(100)))"
                case .image(let image):
                    switch image.source {
                    case .url(let url):
                ...
                case .toolCalls(let toolCallsSegment):
                    return "toolCalls(\(toolCallsSegment.calls.count))"
                }
            }.joined(separator: ", ")
            ...
            logger.info("Tool invocation successful", metadata: ["toolName": "\(tool.name)"])
            results.append(OpenAIToolInvocationResult(call: transcriptCall, output: output))
        } catch {
            logger.error(
